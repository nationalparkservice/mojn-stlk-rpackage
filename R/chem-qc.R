...
ChemLakeNutrientBarPlot <- function(conn, path.to.data, park, site, field.season, data.source = "database") {
    ...
    lake.nut.bar %<>% dplyr::arrange(match(Characteristic_f, c("UTN", "TDN", "NO3NO2-N", "UTP", "TDP", "DOC"), dplyr::desc(Characteristic_f))) %>%
...
ChemStreamNutrientBarPlot <- function(conn, path.to.data, park, site, field.season, data.source = "database") {
    ...
    stream.nut.bar %<>% dplyr::arrange(match(Characteristic_f, c("UTN", "TDN", "NO3NO2-N", "UTP", "TDP", "DOC"), dplyr::desc(Characteristic_f))) %>%
