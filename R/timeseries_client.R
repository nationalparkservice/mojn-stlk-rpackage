  ...
  methods = list(
    ...
    getLocationData = function(locationIdentifier) {
      locationData <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get location data for", locationIdentifier)
      ), "text"))
    },
    ...
    getRatings = function(locationIdentifier, queryFrom, queryTo, inputParameter, outputParameter) {
      if (missing(locationIdentifier)) {
        locationIdentifier <- NULL
      }
      if (missing(inputParameter)) {
        inputParameter <- NULL
      }
      if (missing(outputParameter)) {
      ...
      ratingModels <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get rating models for", locationIdentifier)
    ...
    getRatingModelOutputValues = function(ratingModelIdentifier, inputValues, effectiveTime, applyShifts) {
      if (missing(effectiveTime)) {
        effectiveTime <- NULL
      }
      if (missing(applyShifts)) {
        applyShifts <- NULL
      }

      # Coerce native R dates to an ISO 8601 string
      if (is.double(effectiveTime)) {
        effectiveTime <- .self$formatIso8601(effectiveTime)
      }
      ...
      outputValues <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get rating model output values for", ratingModelIdentifier)
      ), "text"))$OutputValues
    },
    ...
    getFieldVisits = function(locationIdentifier, queryFrom, queryTo, activityType) {
      ...
      visits <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get field visits for", locationIdentifier)
    ...
    getMetadataChangeTransactionList = function(timeSeriesIdentifier, queryFrom, queryTo) {
      ...
      # Coerce native R dates to an ISO 8601 string
      if (is.double(queryFrom)) {
        queryFrom <- timeseries$formatIso8601(queryFrom)
      }
      if (is.double(queryTo)) {
        queryTo <- timeseries$formatIso8601(queryTo)
      }
      ...
      metadataChangeList <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get metadata change list for", timeSeriesIdentifier)
      ), "text"))
    ...
    getTimeSeriesDescriptions = function(locationIdentifier, parameter, publish, computationIdentifier, computationPeriodIdentifier, extendedFilters) {
      ...
      if (missing(computationIdentifier)) {
        computationIdentifier <- NULL
      }
      if (missing(computationPeriodIdentifier)) {
        computationPeriodIdentifier <- NULL
      }
      ...
      timeSeries <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get time-series descriptions for", locationIdentifier)
      ), "text"))$TimeSeriesDescriptions
    },
    ...
    getTimeSeriesData = function(timeSeriesIds, queryFrom, queryTo, outputUnitIds, includeGapMarkers) {
      ...
      if (missing(includeGapMarkers)) {
        includeGapMarkers <- NULL
      }
    ...
    getTimeSeriesCorrectedData = function(timeSeriesIdentifier, queryFrom, queryTo, getParts, includeGapMarkers) {
      ...
      data <- jsonlite::fromJSON(httr::content(httr::stop_for_status(
        ...
        paste("get corrected data for", timeSeriesIdentifier)
      ), "text"))
    },
